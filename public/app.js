...
const analyzeText = (text) => backend.analyze(text);
...
function speak(text) {
  ...
  const engine = localStorage.getItem('n2.tts') || (N2_STATIC ? 'browser' : 'google');
  if (engine === 'browser') return speakBrowser(text);
  const a = new Audio();
  if (backend.ttsNoReferrer) a.referrerPolicy = 'no-referrer';
  a.src = backend.ttsUrl(text);
  currentAudio = a;
  let started = false;
  const fallback = () => { if (currentAudio === a && !started) { a.pause(); currentAudio = null; speakBrowser(text); } };
  a.onplaying = () => { started = true; };
  a.onerror = fallback;
  setTimeout(fallback, 4000); // Google 沒回應（例如被擋）時改用瀏覽器語音
...
function saveDb(immediate = false) {
  ...
  const doSave = () => backend.saveDb(state.db).catch((e) => toast('儲存失敗：' + e.message, true));
...
function renderUnits(root) {
  ...
  if (N2_STATIC) bindSyncCard(root);
  $('#importFile', root).onchange = importJson;
}

// ---------- 跨裝置同步（GitHub Gist，純瀏覽器版） ----------
function syncCardHtml() {
  const token = backend.gist.token();
  const last = +localStorage.getItem('n2.lastSync') || 0;
  return `<div class="card" style="margin-top:20px" id="syncCard">
    <h3>☁️ 跨裝置同步（GitHub Gist）</h3>
    <p class="small muted" style="margin:0 0 10px">這個版本的資料存在瀏覽器裡。要在手機 / 其他電腦看到同一份資料，請到
      <a href="https://github.com/settings/tokens/new?scopes=gist&description=N2%20vocab" target="_blank" rel="noopener">GitHub 建立 Token</a>（只勾 <b>gist</b> 權限），貼到下方。
      資料會存成你帳號底下的一個私密 Gist，每台裝置貼同一組 Token 即可同步。</p>
    <div class="row">
      <input type="text" id="gistToken" class="grow" placeholder="ghp_xxxxxxxx" value="${esc(token)}" autocomplete="off">
      <button class="btn primary" id="gistSave">${token ? '更新 Token' : '啟用同步'}</button>
      ${token ? '<button class="btn" id="gistSync">立即同步</button><button class="btn danger" id="gistOff">停用</button>' : ''}
    </div>
    <div class="small muted" style="margin-top:8px" id="gistStatus">${token ? `已啟用${last ? '，上次同步 ' + fmtDate(last) : ''}` : '尚未啟用（資料只存在這台裝置）'}</div>
  </div>`;
}
function bindSyncCard(root) {
  const status = (msg, err) => { const el = $('#gistStatus', root); if (el) { el.textContent = msg; el.style.color = err ? 'var(--bad)' : ''; } };
  $('#gistSave', root).onclick = async () => {
    const t = $('#gistToken', root).value.trim();
    if (!t) return toast('請貼上 Token', true);
    localStorage.setItem('n2.gistToken', t);
    localStorage.removeItem('n2.gistId');
    status('連線中…');
    try {
      const remote = await backend.gist.pull();
      const lt = (state.db.meta && state.db.meta.updatedAt) || 0;
      const rt = (remote && remote.meta && remote.meta.updatedAt) || 0;
      if (remote && rt > lt && (remote.words || []).length) {
        if (await confirmDialog(`Gist 上已有資料（${remote.units.length} 個單元、${remote.words.length} 個單字），要用它取代這台裝置的資料嗎？（取消＝以這台裝置的資料覆蓋 Gist）`, '使用 Gist 的資料')) {
          state.db = remote; localStorage.setItem('n2.db', JSON.stringify(remote));
        } else await backend.syncNow(state.db);
      } else await backend.syncNow(state.db);
      toast('同步已啟用');
      render();
    } catch (e) { status('連線失敗：' + e.message + '（請確認 Token 正確且有 gist 權限）', true); localStorage.removeItem('n2.gistToken'); }
  };
  const sync = $('#gistSync', root);
  if (sync) sync.onclick = async () => {
    ...
    try {
      ...
      const lt = (state.db.meta && state.db.meta.updatedAt) || 0;
      const rt = (remote && remote.meta && remote.meta.updatedAt) || 0;
      if (remote && rt > lt) { state.db = remote; localStorage.setItem('n2.db', JSON.stringify(remote)); toast('已從 Gist 取得較新的資料'); }
      else { await backend.syncNow(state.db); toast('已上傳到 Gist'); }
      localStorage.setItem('n2.lastSync', String(Date.now()));
      render();
    } catch (e) { status('同步失敗：' + e.message, true); }
  };
  const off = $('#gistOff', root);
  if (off) off.onclick = () => { localStorage.removeItem('n2.gistToken'); localStorage.removeItem('n2.gistId'); render(); };
}
...
async function openDetail(id) {
  ...
  const body = openModal(`
    ...
    <div class="detail-section"><h3>📗 ${N2_STATIC ? 'Jotoba 辭典' : 'Jisho 辭典'} <span class="badge gray">英日 · JLPT 等級</span></h3><div id="dJisho" class="loading">載入中…</div></div>
    <div class="detail-section"><h3>📘 Wiktionary 日日解釋</h3><div id="dWikt" class="loading">載入中…</div></div>
    <div class="detail-section"><h3>💬 ${N2_STATIC ? 'Jotoba 例句 <span class="badge gray">附英文翻譯</span>' : 'Tatoeba 例句 <span class="badge gray">附中文翻譯</span>'}</h3><div id="dTatoeba" class="loading">載入中…</div></div>
  `);
  $$('[data-speak]', body).forEach((b) => (b.onclick = () => speak(b.dataset.speak)));

  const q = affixCore(w.word) || w.word;
  backend.jisho(q).then((r) => {
...
async function init() {
  ...
  if (N2_STATIC) {
    ...
    ind.id = 'syncInd'; ind.className = 'small muted';
    $('.unit-picker').appendChild(ind);
    backend.onSync((st) => {
      if (st.startsWith('dict:')) { ind.textContent = st.slice(5) ? '⏳ ' + st.slice(5) : ''; return; }
      ind.textContent = st === 'syncing' ? '☁️ 同步中…' : st === 'pending' ? '☁️ 待同步' : st === 'ok' ? '☁️ 已同步' : st.startsWith('error:') ? '⚠️ 同步失敗' : '';
      ind.title = st.startsWith('error:') ? st.slice(6) : '';
    });
  }
  try {
    state.db = await backend.loadDb();
